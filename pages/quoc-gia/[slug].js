    ...
    return (
        <><Header menuData={menuData} /><div className="container">
            ...
            <main style={{padding: '100px 4%', color: 'white'}}>
                <h1 style={{textAlign: 'center', marginBottom: '2rem'}}>Quốc gia: {termInfo.name}</h1>
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '20px'}}>
                    {movies.map(movie => <MovieCard key={movie.id} movie={movie} />)}
                </div>
                <Pagination currentPage={currentPage} totalPages={totalPages} basePath={`/quoc-gia/${termInfo.slug}`} />
            </main>
...
export async function getServerSideProps(context) {
    ...
    const termInfo = menuData?.regions.find(item => item.slug === slug);
    if (!termInfo) return { notFound: true };

    const apiResult = await getMoviesByTerm('ophim_regions', termInfo.id, currentPage);
